...
The data from the survey has been saved in a `.csv` file. This file is read into R using `read.csv`. Before reading the `CSV` file, you will save it to your local computer. These data are available in [crabbieMRDS.csv](crabbieMRDS.csv); use right mouse button on link to save `crabbieMRDS.csv` to your local drive so you can access the MRDS file.
...
We can also analyse the crabeater seals data as if it were single platform data (i.e. ignoring that $p(0)$ is less than 1). These data are available in [crabbieMCDS.csv](crabbieMCDS.csv); use right mouse button on link to save `crabbieMCDS.csv` to your local drive so you can access the MCDS file.
