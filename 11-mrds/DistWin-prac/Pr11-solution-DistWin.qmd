...
These solutions are applicable to both the Distance in Windows and mrds R versions.
...
(Note that there are differences in the measures of precision between the Distance in Windows version and the `mrds` in R version because the number of transect lines (samples) differed between the two sets of data. Other statistics are the same.)

## 1. and 2. Estimation of *p*; FI - MR dist and FI - MR dist+size+sex+exp
